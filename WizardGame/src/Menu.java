...
public class Menu {
	...
	public Rectangle restart = new Rectangle(2*Game.WIDTH/3 + 100, 180, 150, 60);
	...
	public void render(Graphics g) {
		...
		if(Game.getState() == Game.STATE.MENU && Game.isStart == false) {
			g.drawString("Wizard Game", 2*Game.WIDTH/3, 100);
		}
		Font font1 = new Font("arial", Font.BOLD, 30);
		g.setFont(font1);
		if(Game.getState() == Game.STATE.MENU && Game.isStart == true) {
			g.drawString("Continue", cont.x + 10, cont.y+ 40);
			g2d.draw(cont);
			g.drawString("Restart", restart.x + 10, restart.y + 40);
			g2d.draw(restart);
			...
			g.drawString("Main Menu", quit.x + 10, quit.y + 40);
			g2d.draw(settings);
			g2d.draw(play);
			g2d.draw(quit);
